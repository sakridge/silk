...
fn run_cluster_partition(
    ...
    warn!("PARTITION_TEST done sleeping until partition start timeout");
    let now = timestamp();
    ...
    while !done {
        for (i, ingress_node) in alive_node_contact_infos.iter().enumerate() {
            ...
            warn!("PARTITION_TEST min observed roots {}/16", min_node);
            done = min_node >= 16;
        }
        sleep(Duration::from_millis(clock::DEFAULT_MS_PER_SLOT / 2));
    }
    warn!("PARTITION_TEST done spending on all node");
}
