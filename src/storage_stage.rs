...
impl StorageStage {
    pub fn new(
        ...
        //let ledger_path = String::from(ledger_path);
        let ledger_path = if let Some(ledger_str) = ledger_path {
            Some(String::from(ledger_str))
        } else {
            None
        };
    ...
    pub fn process_entry_crossing(
        ...
        #[cfg(feature = "cuda")]
        {
            ...
            match chacha_cbc_encrypt_file_many_keys(
                ...
                Err(e) => info!("error encrypting file: {:?}", e),
            }
        }
        Ok(())
    }
                ...
                for tx in entry.transactions {
                    for program_id in tx.program_ids {
                        if VoteProgram::check_id(&program_id) {
                            debug!("generating storage_keys from votes current_key_idx: {}", *current_key_idx);
                            let mut storage_keys = storage_keys.write().unwrap();
                            storage_keys[*current_key_idx..*current_key_idx + size_of::<Signature>()].copy_from_slice(tx.signature.as_ref());
...
mod tests {
    ...
    fn test_storage_stage_process_vote_entries() {
        ...
        let vote = Vote { tick_height: 123456 };
        let keypair = Keypair::new();
